...
if (root) {
  ...
  const names = { 'news-guarded': 'AI + news', 'price-only': 'Price AI', reference: 'Buy & hold' };
  const numbers = { 'news-guarded': '01', 'price-only': '02', reference: '03' };
  ...
  const runName = run => run.startsWith('paper-v2-') ? 'Common start · v2' : 'Cash start · v1';
  let loading = false;
  ...
  function marker(g, account, x, y) {
    ...
    if (account === 'news-guarded') g.append(svg('circle', { ...attrs, cx: x, cy: y, r: 4.5 }));
    else if (account === 'price-only') g.append(svg('rect', { ...attrs, x: x - 4.5, y: y - 4.5, width: 9, height: 9 }));
  ...
  function chart() {
    ...
    for (const h of histories) {
      ...
      g.append(svg('path', { d: path, class: `equity-series ${h.account}`, 'data-series': h.account }));
    ...
    const first = data.history.find(h => h.account === 'news-guarded').points;
    const second = data.history.find(h => h.account === 'price-only').points;
    ...
    text('chart-explanation', chartMode === 'selected' ? `Showing ${numbers[selected]} · ${names[selected]} on its own, against the cash baseline. The scale stays the same so you can compare fairly.` : allOverlap ? 'All three portfolios have identical recorded values, so their lines overlap. Select a portfolio and choose “Selected portfolio” to inspect its line.' : overlap ? 'AI + news and Price AI have identical recorded values, so their lines overlap. Choose a portfolio above, then “Selected portfolio” to see its line on its own.' : 'Blue circles: AI + news. Orange squares: Price AI. Purple triangles: Buy & hold. The grey zero line shows what keeping all $10,000 in cash would return.');
    ...
    root.querySelectorAll('[data-legend]').forEach(n => { n.hidden = chartMode !== 'all' && n.dataset.legend !== selected; });
  ...
  function latestDecisions(a) {
    ...
    const cards = latest.map(d => {
      ...
      if (d.signal && a.id !== 'reference' && !setupDecision(d)) {
      ...
      card.append(node('p', `${passed}/7 market checks passed${d.account === 'news-guarded' ? d.news.veto ? ' · news veto recorded' : d.news.state === 'unavailable' ? ' · news unknown' : d.news.state === 'partial' ? ' · partial news coverage, no veto' : ' · no news veto' : ''}`, 'decision-checks'));
  ...
  function audit(d) {
    ...
    item.append(summary);
    const body = node('div', undefined, 'audit-body'); body.append(node('p', reason[d.reason]));
    if (d.signal && d.account !== 'reference' && !setupDecision(d)) body.append(node('p', `Forecast move: ${d.signal.expected_return === null ? 'unavailable' : signedPct(d.signal.expected_return)}. A new buy needs more than ${pct(d.signal.entry_threshold)}.`, 'decision-explainer'));
    if (d.signal) { const forecastLink = node('a', 'View the frozen forecast & its outcome', 'text-link'); forecastLink.href = '/forecasts/?id=' + encodeURIComponent(d.signal.forecast_id); body.append(forecastLink); }
    const technical = node('details', undefined, 'technical-evidence'); technical.append(node('summary', 'Inspect market checks, model inputs and news evidence'));
    ...
    columns.append(left, right); evidence.append(columns, node('h3', '3. News evidence & execution'));
    const newsMessage = d.news.state === 'unavailable' ? 'No eligible JEV evidence is available. This is unknown, not neutral; the fixed price rules may still proceed.' : `${d.news.items.length} eligible JEV annotation(s), coverage ${d.news.state}. ${d.news.veto ? 'An adverse event triggers the JEV guard.' : 'No annotation triggers the JEV guard.'}`;
    evidence.append(node('p', newsMessage + (setupDecision(d) ? ' This is context only for the fixed common start; the news guard applies to subsequent AI buys.' : d.account !== 'news-guarded' ? ' This portfolio does not apply the news guard.' : ''), 'fine-print'));
    ...
    body.append(node('p', execution)); evidence.append(node('p', 'JEV scores classify supplied headlines; they are not calibrated probabilities of market outcomes.', 'fine-print'));
    evidence.append(node('p', `Decision #${d.seq} · SHA-256 ${d.hash}\nObservation ${d.quality.observation_id}${d.signal ? `\nForecast ${d.signal.forecast_id}\nInputs ${d.signal.input_hash}` : ''}`, 'audit-provenance'));
  ...
  function render() {
    ...
    root.querySelectorAll('[data-portfolio]').forEach(card => {
      const account = data.accounts.find(v => v.id === card.dataset.portfolio);
    ...
    text('drawdown', pct(a.max_drawdown)); text('brake', id === 'reference' ? 'Measured from its highest value' : a.braked ? '8% limit reached · new buys paused' : '8% fall pauses new buys');
    ...
    rows($('trades'), trades.map(t => [shortDate(t.at), `${t.side === 'buy' ? 'Bought' : 'Sold'} ${t.asset}`, t.quantity.toFixed(8), money(t.price), money(t.gross), money(t.fee)]));
  ...
  root.querySelectorAll('[data-portfolio]').forEach(button => button.addEventListener('click', () => { selected = button.dataset.portfolio; historyLimit = 6; $('action').value = 'all'; render(); }));
