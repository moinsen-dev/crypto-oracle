...
export const models = ['timesfm', 'calibrated', 'fusion_market', 'fusion_news'];
...
function assetHorizon(o) { member(o.asset, assets); member(o.horizon, [24, 72]); }
...
export function validateRecord(r, now) {
  ...
  assetHorizon(c); member(c.model, models); member(c.experiment, [policy.parent, experiment]);
  ...
  list(c.path, 72); ensure(c.path.length === c.horizon);
  c.path.forEach((p, i) => { keys(p, 't p'); ensure(p.t === c.origin + (i + 1) * 3600); num(p.p, 1e-12); });
  ...
  list(r.baselines, 3); const names = new Set();
  for (const b of r.baselines) { keys(b, 'model prediction error'); member(b.model, ['timesfm', 'persistence', 'momentum']); ensure(!names.has(b.model)); names.add(b.model); num(b.prediction, 1e-12); nullable(b.error); if (b.error !== null) { ensure(e); close(b.error, Math.abs(Math.log(b.prediction / e.actual))); } }
  return r;
}

export function validateForecasts(d) {
  keys(d, 'schema generated_at experiment started_at policy groups learning news records'); ensure(d.schema === 1 && d.experiment === experiment);
